...
rendering_ratio (nano::mDyna_ratio),
node (node_a),
wallet_m (wallet_a),
account (account_a),
...
std::string nano_qt::wallet::format_balance (nano::uint128_t const & balance) const
{
	...
	if (rendering_ratio == nano::kDyna_ratio)
	{
		unit = std::string ("knano");
	}
	else if (rendering_ratio == nano::dyna_ratio)
	{
...
bootstrap_label (new QLabel ("IPV6:port \"::ffff:192.168.0.1:7085\"")),
...
wallet (wallet_a)
{
	...
	QObject::connect (mnano_unit, &QRadioButton::toggled, [this]() {
		if (mnano_unit->isChecked ())
		{
			...
			this->wallet.change_rendering_ratio (nano::mDyna_ratio);
		}
	});
	QObject::connect (knano_unit, &QRadioButton::toggled, [this]() {
		if (knano_unit->isChecked ())
		{
			...
			this->wallet.change_rendering_ratio (nano::kDyna_ratio);
		}
	});
	QObject::connect (nano_unit, &QRadioButton::toggled, [this]() {
		if (nano_unit->isChecked ())
		{
			...
			this->wallet.change_rendering_ratio (nano::dyna_ratio);
		}
	});
