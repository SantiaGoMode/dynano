		...
		if (!nano::try_read (weight_stream, block_height))
		{
			...
			if (ledger.store.block_count (transaction).sum () < max_blocks)
			{
				...
				while (true)
				{
					...
					BOOST_LOG (log) << "Using bootstrap rep weight: " << account.to_account () << " -> " << weight.format_balance (mDyna_ratio, 0, true) << " DYNA";
...
{
	assert (balance_a >= amount_a * nano::gDyna_ratio);
	auto balance_l (balance_a);
	double result (0.0);
	for (auto i (0); i < amount_a; ++i)
	{
		balance_l -= nano::gDyna_ratio;
		auto balance_scaled ((balance_l / nano::mDyna_ratio).convert_to<double> ());
