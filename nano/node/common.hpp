...
public:
	...
	static std::array<uint8_t, 2> constexpr magic_number = nano::is_test_network ? std::array<uint8_t, 2>{ { 'D', 'Z' } } : nano::is_beta_network ? std::array<uint8_t, 2>{ { 'D', 'Y' } } : std::array<uint8_t, 2>{ { 'D', 'X' } };
	uint8_t version_max;
	...
	{
		return magic_number[0] == 'B' && magic_number[1] >= 'X' && magic_number[1] <= 'Z';
	}
	bool valid_network () const
	{
		return (magic_number[1] - 'Z') == static_cast<int> (nano::nano_network);
	}
};
class message
{
