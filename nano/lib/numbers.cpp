...
void nano::uint256_union::encode_account (std::string & destination_a) const
{
	...
	destination_a.append ("_anyd"); // xrb_
...
bool nano::uint256_union::decode_account (std::string const & source_a)
{
	...
	if (!error)
	{
		auto dyna_prefix (source_a[0] == 'd' && source_a[1] == 'y' && source_a[2] == 'n' && source_a[3] == 'a' && (source_a[4] == '_' || source_a[4] == '-'));
		error = (dyna_prefix && source_a.size () != 65);
		if (!error)
		{
			if (dyna_prefix)
			{
				auto i (source_a.begin () + (dyna_prefix ? 4 : 5));
				if (*i == '1' || *i == '3')
				{
