...
TEST (rpc, history)
{
	...
	nano::genesis genesis;
	nano::state_block usend (nano::genesis_account, node0->latest (nano::genesis_account), nano::genesis_account, nano::genesis_amount - nano::gDyna_ratio, nano::genesis_account, nano::test_genesis_key.prv, nano::test_genesis_key.pub, 0);
	...
	ASSERT_EQ (nano::gDyna_ratio.convert_to<std::string> (), std::get<2> (history_l[0]));
	ASSERT_EQ (5, history_l.size ());
	...
	ASSERT_EQ (nano::gDyna_ratio.convert_to<std::string> (), std::get<2> (history_l[1]));
...
TEST (rpc, process_subtype_send)
{
	...
	auto latest (system.nodes[0]->latest (nano::test_genesis_key.pub));
	auto & node1 (*system.nodes[0]);
	nano::state_block send (nano::genesis_account, latest, nano::genesis_account, nano::genesis_amount - nano::gDyna_ratio, key.pub, nano::test_genesis_key.prv, nano::test_genesis_key.pub, node1.work_generate_blocking (latest));
	nano::rpc rpc (system.io_ctx, node1, nano::rpc_config (true));
	rpc.start ();
	boost::property_tree::ptree request;
	request.put ("action", "process");
...
TEST (rpc, process_subtype_open)
{
	...
	nano::state_block open (key.pub, 0, key.pub, nano::gDyna_ratio, send.hash (), key.prv, key.pub, node1.work_generate_blocking (key.pub));
	nano::rpc rpc (system.io_ctx, node1, nano::rpc_config (true));
	rpc.start ();
	boost::property_tree::ptree request;
	request.put ("action", "process");
...
TEST (rpc, process_subtype_receive)
{
	...
	auto latest (system.nodes[0]->latest (nano::test_genesis_key.pub));
	auto & node1 (*system.nodes[0]);
	nano::state_block send (nano::genesis_account, latest, nano::genesis_account, nano::genesis_amount - nano::gDyna_ratio, nano::test_genesis_key.pub, nano::test_genesis_key.prv, nano::test_genesis_key.pub, node1.work_generate_blocking (latest));
	{
		auto transaction (node1.store.tx_begin_write ());
		ASSERT_EQ (nano::process_result::progress, node1.ledger.process (transaction, send).code);
	}
...
TEST (rpc, payment_wait)
{
	...
	request1.put ("amount", nano::amount (nano::mDyna_ratio).to_string_dec ());
	...
	system.wallet (0)->send_action (nano::test_genesis_key.pub, key.pub, nano::mDyna_ratio);
	...
	request1.put ("amount", nano::amount (nano::mDyna_ratio * 2).to_string_dec ());
	test_response response3 (request1, rpc, system.io_ctx);
	system.deadline_set (5s);
	while (response3.status == 0)
	{
...
TEST (rpc, md_to_raw)
{
	...
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (nano::mDyna_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}

TEST (rpc, md_from_raw)
{
	nano::system system (24000, 1);
	nano::node_init init1;
	auto & node1 (*system.nodes[0]);
	...
	rpc.start ();
	boost::property_tree::ptree request1;
	request1.put ("action", "md_from_raw");
	request1.put ("amount", nano::mDyna_ratio.convert_to<std::string> ());
	test_response response1 (request1, rpc, system.io_ctx);
	system.deadline_set (5s);
	while (response1.status == 0)
	{
...
TEST (rpc, kd_to_raw)
{
	...
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (nano::kDyna_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}

TEST (rpc, kd_from_raw)
{
	nano::system system (24000, 1);
	nano::node_init init1;
	auto & node1 (*system.nodes[0]);
	...
	rpc.start ();
	boost::property_tree::ptree request1;
	request1.put ("action", "kd_from_raw");
	request1.put ("amount", nano::kDyna_ratio.convert_to<std::string> ());
	test_response response1 (request1, rpc, system.io_ctx);
	system.deadline_set (5s);
	while (response1.status == 0)
	{
...
TEST (rpc, dyna_to_raw)
{
	...
	rpc.start ();
	boost::property_tree::ptree request1;
	request1.put ("action", "dyna_to_raw");
	request1.put ("amount", "1");
	test_response response1 (request1, rpc, system.io_ctx);
	system.deadline_set (5s);
	while (response1.status == 0)
	...
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (nano::dyna_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}

TEST (rpc, dyna_from_raw)
{
	nano::system system (24000, 1);
	nano::node_init init1;
	auto & node1 (*system.nodes[0]);
	...
	rpc.start ();
	boost::property_tree::ptree request1;
	request1.put ("action", "dyna_from_raw");
	request1.put ("amount", nano::dyna_ratio.convert_to<std::string> ());
	test_response response1 (request1, rpc, system.io_ctx);
	system.deadline_set (5s);
	while (response1.status == 0)
	{
...
TEST (rpc, block_create_state)
{
	...
	request.put ("balance", (nano::genesis_amount - nano::gDyna_ratio).convert_to<std::string> ());
...
TEST (rpc, block_create_state_open)
{
	...
	nano::genesis genesis;
	system.wallet (0)->insert_adhoc (nano::test_genesis_key.prv);
	auto send_block (system.wallet (0)->send_action (nano::test_genesis_key.pub, key.pub, nano::gDyna_ratio));
	...
	request.put ("representative", nano::test_genesis_key.pub.to_account ());
	request.put ("balance", nano::gDyna_ratio.convert_to<std::string> ());
...
TEST (rpc, online_reps)
{
	...
	auto send_block (system.wallet (0)->send_action (nano::test_genesis_key.pub, nano::test_genesis_key.pub, nano::gDyna_ratio));
	ASSERT_NE (nullptr, send_block);
	system.deadline_set (10s);
...
TEST (rpc, confirmation_history)
{
	...
	assert (tally_num == nano::genesis_amount || tally_num == (nano::genesis_amount - nano::gDyna_ratio));
	system.stop ();
}
...
TEST (rpc, confirmation_history_hash)
{
	...
	system.wallet (0)->insert_adhoc (nano::test_genesis_key.prv);
	ASSERT_TRUE (system.nodes[0]->active.list_confirmed ().empty ());
	auto send1 (system.wallet (0)->send_action (nano::test_genesis_key.pub, key.pub, nano::gDyna_ratio));
	auto send2 (system.wallet (0)->send_action (nano::test_genesis_key.pub, key.pub, nano::gDyna_ratio));
	auto send3 (system.wallet (0)->send_action (nano::test_genesis_key.pub, key.pub, nano::gDyna_ratio));
	system.deadline_set (10s);
	...
	nano::amount tally_num;
	tally_num.decode_dec (tally);
	assert (tally_num == nano::genesis_amount || tally_num == (nano::genesis_amount - nano::gDyna_ratio) || tally_num == (nano::genesis_amount - 2 * nano::gDyna_ratio) || tally_num == (nano::genesis_amount - 3 * nano::gDyna_ratio));
	system.stop ();
}
...
TEST (rpc, block_confirm)
{
	...
	nano::genesis genesis;
	system.wallet (0)->insert_adhoc (nano::test_genesis_key.prv);
	auto send1 (std::make_shared<nano::state_block> (nano::test_genesis_key.pub, genesis.hash (), nano::test_genesis_key.pub, nano::genesis_amount - nano::gDyna_ratio, nano::test_genesis_key.pub, nano::test_genesis_key.prv, nano::test_genesis_key.pub, system.nodes[0]->work_generate_blocking (genesis.hash ())));
	{
		auto transaction (system.nodes[0]->store.tx_begin (true));
...
TEST (rpc, sign_hash)
{
	...
	nano::keypair key;
	auto & node1 (*system.nodes[0]);
	nano::state_block send (nano::genesis_account, node1.latest (nano::test_genesis_key.pub), nano::genesis_account, nano::genesis_amount - nano::gDyna_ratio, key.pub, nano::test_genesis_key.prv, nano::test_genesis_key.pub, 0);
	nano::rpc rpc (system.io_ctx, node1, nano::rpc_config (true));
	rpc.start ();
	boost::property_tree::ptree request;
