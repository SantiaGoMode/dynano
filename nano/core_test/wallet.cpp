...
TEST (wallet, send_race)
{
	...
	for (auto i (1); i < 60; ++i)
	{
		ASSERT_NE (nullptr, system.wallet (0)->send_action (nano::test_genesis_key.pub, key2.pub, nano::gDyna_ratio));
		ASSERT_EQ (nano::genesis_amount - nano::gDyna_ratio * i, system.nodes[0]->balance (nano::test_genesis_key.pub));
	}
}
